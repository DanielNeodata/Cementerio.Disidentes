...
function showReport() {
	...
	_AJAX.UiGetPlanDeCuentas(myObj).then(function (datajson) {
		var _html = "";
		...
		$.each(datajson.cuentas, function (j, val1) {
			...
			_html += "<tr><td style='text-align:left;'>" + val1.IndentHtml + val1.NUMERO + "</td><td style='text-align:left;'>" + val1.IndentHtml+ val1.NOMBRE + "</td>";

			if (tipo == "S") {
				_html += "<td>" + val1.TipoAjuste + "</td>";
			}
			if (saldo == "S") {
				//_html += "<td>" + _TOOLS.formatNumber("es",val1.SALDONOMIN,"N") + "</td><td>" + val1.SALDOAJUST + "</td>";
				_html += "<td>" + _TOOLS.showNumber(val1.SALDONOMIN, 2, ",", "") + "</td><td>" + _TOOLS.showNumber(val1.SALDOAJUST, 2, ",", "") + "</td>";
			}
