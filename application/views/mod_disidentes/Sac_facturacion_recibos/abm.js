...
function showReport() {
	...
	if (recibo == "") {alert("No hay un nro de recibo que mostrar....");}
	...
	_AJAX.UiGetRecibo(myObj).then(function (datajson) {
		...
		_html += "}";
		_html += "</style>";
		//_html += "<div id='divPrinter' style='position:absolute;left:0;top:0mm;'><img class='imgPrint' src='./assets/img/print.jpg' onclick=javascript:$('#divPrinter').hide();self.print() width='32'/></div>";
		_html += "<div id='divPrinter' style='position: absolute; left: 0; top: 0mm;'><img src='" + _AJAX._here + "/assets/img/print.jpg' style='height:35px;' onclick=\"javascript: document.getElementById('divPrinter').style.display = 'none'; self.print();\"/></div>";
		...
		case 3:
			...
			_AJAX.UiGetLote(myObj).then(function (datajson) {
				...
				if (estOcup=="REVIS" || estOcup=="ARREN") {
					...
					//alert("tito:::::::::::: "+datajson.data[0].TITULO);
					
					//if (datajson.data[0].TITULO==null){alert("tito is nullllllllllll");}
...
function insertPagoACuentaYAlertas(idlote, oper) {
	...
	_AJAX.UiGetLote(myObj).then(function (datajson) {
		...
		if (recibo > 0) {
			var k = 1;
			if ($(".pcagregado").val() == "SI") { return false; }
			rows += 1;
			...
			_html += "		<input type=text class='pcagregado form-control text dbase' style='display: none' id='detail-pcagregado-" + rows + "' name='detail-pcagregado-" + rows + "' value='SI'></input>";
			_html += "	</td>";
			_html += "</tr>";
			$("#OPER-COUNTER").val(rows);
			$("#TOTAL-1").val(total);
		...
		if (datajson.data[0].TITULO == "S") {textW += "&nbsp;&nbsp;<strong><span style=\"color:blue;\">Hay un título para entregar</span></strong>";}
		if (datajson.data[0].REGLAMENTO == "S") {textW += "&nbsp;&nbsp;<strong><span style=\"color:blue;\">Entregar Reglamento Interno</span></strong>";}
		$("#txtWarning").html(textW);
	});
}
	...
	switch (idOption) {
		...
		case 2:
			...
			_AJAX.UiGetLote(myObj).then(function (datajson) {
				...
				if (!(datajson.data[0].ANOSRENOVA == "" || datajson.data[0].ANOSRENOVA == "null" || datajson.data[0].ANOSRENOVA == null)) { anosrenova = 0; }
				...
				let totn = prompt("Desea modificar el valor calculado?", tot);
				try {
					importeN = totn;
					if (importeN == "") { alert("Debe ingresar un número en importe mayor o igual a cero"); return; }
					importeN = Number(importeN);
					if (isNaN(importeN)) { alert("Debe ingresar un número en importe mayor o igual a cero"); return; }
					if (importeN < 0) { alert("El número en importe tiene que ser mayor o igual a cero"); return; }
				} catch (err) {
					alert("Error al buscar número en importe");
					return;
				}
		...
		case 3:
			...
			if (typeof rblArr === "undefined") { rblArr = "S"; }
			if (rblArr == "N") {
			...
			if (titulo===undefined || reglamento===undefined){alert("Ni el título ni el reglamento pueden estar sin seleccionar una opción. Seleccione y vuelva a intentar..."); return;}
			const fechaHoy = new Date();
			...
			var total = Number($("#TOTAL-1").val());
			let totn = prompt("Desea modificar el valor calculado?", valor);
			try {
				importeN = totn;
				if (importeN == "") { alert("Debe ingresar un número en importe mayor o igual a cero"); return; }
				importeN = Number(importeN);
		...
		case 4:
			...
			var total = Number($("#TOTAL-1").val());
			totn1 = prompt("Desea modificar el valor calculado?", valor);
			try {
				importeN = totn1;
				if (importeN == "") { alert("Debe ingresar un número en importe mayor o igual a cero"); return; }
				importeN = Number(importeN);
		...
		case 5:
			...
			if (optionName1 == "Traslado Interno") {
				...
				con = "Traslado Interno  de Sección " + seccion + " Sepultura Nº " + sepultura + "\n" + " a Sección " + seccion2 + " Sepultura " + sepultura2 + ".";
				OP = "TI";
				...
				if (tipo == "UA" || tipo == "UR" || tipo == "UC") {
					con = "Traslado Externo  de " + "Sección " + seccion + " " + "Sepultura Nº " + sepultura + " \n" + "Apertura " + apertura + " " + "URNA" + " " +nombre ;
				} else {
		...
		case 6:
			...
			var nombre = cadena[3].replace(/_/g, "");
			var tipo = cadena[2].replace(/ /g, "");
			var apertura = parseInt(cadena[4].replace(/_/g, "").replace(/ /g, "").trim());
			var idlote = cadena[5].replace(/_/g, "").replace(/ /g, "");
			var titular = cadena[6].replace(/_/g, "").replace(/ /g, "");
			var cantidad = "";
			if (tipo == "UR" || tipo == "UA") { alert("ATENCION: ya es una urna"); return; }
			if (apertura == 1) {
				valor = obj.RED_1; cantidad = "una";
			}
			else if (apertura == 2) {
				valor = obj.RED_2; cantidad = "dos";
			}
			else {
				valor = obj.RED_3; cantidad = "tres";
			}
		...
		case 7:
			...
			var total = Number($("#TOTAL-1").val());
			var totn4 = prompt("Desea modificar el valor calculado?", valor);
			try {
				importeN = totn4;
				if (importeN == "") { alert("Debe ingresar un número en importe mayor o igual a cero"); return; }
				importeN = Number(importeN);
		...
		case 8:
			...
			var total = Number($("#TOTAL-1").val());
			var totn3 = prompt("Desea modificar el valor calculado?", valor);
			try {
				importeN = totn3;
				if (importeN == "") { alert("Debe ingresar un número en importe mayor o igual a cero"); return; }
				importeN = Number(importeN);
		...
		case 9:
			...
			try {
				importe = $("#TB-importe").val();
				if (importe == "") { alert("Debe ingresar un número en importe"); return; }
				importe = Number(importe);
				if (isNaN(importe)) { alert("Debe ingresar un número en importe"); return; }
			} catch (err) {
				alert("Error al buscar número en importe");
				return;
			}
		...
		case 11:
			...
			var valor = 0;
			if (tiene == "S" && dis == "S") {valor = obj.CENIZAS1;}
			else if (tiene== "S" && dis == "N") {valor = obj.CENIZAS3;}
			else if (tiene == "N" && dis == "S") {valor = obj.CENIZAS2;}
			else if (tiene == "N" && dis == "N") {valor = obj.CENIZAS4;}
			else {valor = 0;}
			var con = "Inhumación de Cenizas en \n el Jardín de los Recuerdos";
			var total = Number($("#TOTAL-1").val());
			var totn2 = prompt("Desea modificar el valor calculado?", valor);
			try {
				importeN = totn2;
				if (importeN == "") { alert("Debe ingresar un número en importe mayor o igual a cero"); return; }
				importeN = Number(importeN);
		...
		case 12:
			var valor = 0;
			try {
				valor = $("#TB-importe").val();
				if (valor == "") { alert("Debe ingresar un número en importe"); return; }
				valor = Number(valor);
				if (isNaN(valor)) { alert("Debe ingresar un número en importe"); return; }
			} catch (err) {
				alert("Error al buscar número en importe");
				return;
			}
		...
		case 15:
			...
			var total = Number($("#TOTAL-1").val());
			total = total + importe;
			detalle = "Renovación a Cuenta de " + "Sección " + seccion + " " + "Sepultura Nº " + sepultura + " \n" + ":  " + detalle ;
			_html = "";
			_html += "<tr id='detail-form-row-" + rows + "'>";
			_html += "   <td><a href='#' onClick=\"javascript:deleteRow('detail-form-row-" + rows + "','detail-imp-" + rows + "','" + rows + "');return false;\">eliminar</a></td>";
			_html += "   <td id='detail-form-row-" + rows + "-id'>" + rows + "</td>";
