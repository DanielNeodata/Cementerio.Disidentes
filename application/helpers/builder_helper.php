...
function buildHeaderBrowStd($parameters,$title){
	...
	if (!isset($parameters["getters"]["processDeleteEmails"])) {$parameters["getters"]["processDeleteEmails"] = false;}
	...
	if ($parameters["getters"]["processBatchMail"]) {
		$html .= "<button class='btn btn-secondary btn-sm btn-mail-search' type='button' " . $dataRec . " data-mode='mailall' data-page='1' data-filters='" . json_encode($parameters['filters']) . "'><i class='material-icons' style='font-size:22px;vertical-align:middle;'>mail</i>Enviar Mail</button>";
	}
	if ($parameters["getters"]["processDeleteEmails"]) {
		$html .= "<button class='btn btn-danger btn-sm btn-mail-deleteall' type='button'><i class='material-icons' style='font-size:22px;vertical-align:middle;'>delete</i>Borrar todos</button>";
	}
	$html.="    </div>";
    $html.="   </div>";
    $html.="</div>";
