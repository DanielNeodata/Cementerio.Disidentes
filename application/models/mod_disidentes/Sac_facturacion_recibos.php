    ...
    public function GetEstadisticasGenerales($values){
	    try {
            ...
            if ($values["DETALLADO"]=="S")
            {
               ...
               $sql = $sql.= "from [SAC_EstaTemp] t left join SAC_EstaLote l on (t.BIMESTRE=l.BIMESTRE) where t.BIMESTRE<=".$values["HASTA"]." and t.BIMESTRE>=".$values["DESDE"]." ORDER BY t.BIMESTRE";
            }
            else
            {
               $sql = " select t.[BIMESTRE],t.[CANTIDAD],t.[IMPORTE],t.[PORCENT1],t.[PORCENT2] ";
               $sql = $sql.= "from [SAC_EstaTemp] t  where t.BIMESTRE<=".$values["HASTA"]." and t.BIMESTRE>=".$values["DESDE"]." ORDER BY t.BIMESTRE";
            }
    ...
    public function offline($values){
        try {
			...
			try
			{
                ...
                $sql = " exec sp_EliminarRecibo ".$val."";

                log_message('error', 'cco-> pasando x se pasaron a eliminados los movimientos de recibos VIEW es:->'.$sql."<-");

                $renovaciones = $this->execAdHocAsArray($sql);

                log_message('error', 'cco-> se pasaron correctamente a eliminados los movimientos de recibos VIEW es:->'.$sql."<-");
    ...
    public function brow($values){
        try {
            ...
            $values["columns"]=array(
                ...
                array("field"=>"offline","format"=>"date"),
    ...
    public function save($values,$fields=null){
		...
		try {
            ...
            for ($i = 1; $i <= $counter; $i++) {
                ...
                else if ($oper=="TI" && strpos($con, "Traslado Interno")>=0)
                {
            ...
            log_message("error", "ABORTE X EXCEPTION: ".$e); 
            return array(
                "code"=>"3000",
                "status"=>"ERROR",
                "message"=>$message,
