...
class Sac_emails extends MY_Model {
    ...
    public function processBatchMail($values){
        try {
		    if(!isset($values["wait"])){$values["wait"]=10;}
		    if(!isset($values["mark"])){$values["mark"]=0;}
			if ((int)$values["mark"]==1){
			   $sql="UPDATE emails SET mark=1 WHERE mark!=1";
			   $this->execAdHoc($sql);
			}
		    $fire=false;
		    $CORREO=$this->createModel(MOD_EMAIL,"emailing","emailing");
            if ($values["where"]!=""){$values["where"]=base64_decode($values["where"]);}
            $where2 = $values["where"];
            $notificaciones=$this->execAdHocAsArray("SELECT TOP 25 * FROM emails WHERE (mark=1) AND (estado='' OR estado IS null OR estado='error') ORDER BY 1 ASC ");
            $server=$this->execAdHocAsArray("SELECT TOP 1 * FROM mailers ");
            /*procesar los mails*/
            $mail = new PHPMailer();
			$config = Array(
				"protocol" => "SMTP",
				"smtp_host" => $server[0]["Direccion"],
				"smtp_timeout" => 25,
				"smtp_port" => (int)$server[0]["Puerto"],
				"smtp_user" => $server[0]["usuario"],
				"smtp_pass" => $server[0]["clave"],
				"mailtype"  => "plain",
				"wordwrap"  => true,
				"charset"   => "utf-8",
				"newline"   => "\r\n"
             ...
             foreach($notificaciones as $r){
				...
				$sended=$CORREO->send($params,$config);
		        if($sended["status"]=="OK") {
                    $stat=$this->execAdHocAsArray("exec sp_emails_update ".$r["id"].",''");
		        }
                else
                {
                    $stat=$this->execAdHocAsArray("exec sp_emails_update ".$r["id"].",'".$sended["message"]."'");
                }
            }
            return array(
                "code"=>"2000",
    ...
    public function save($values,$fields=null){
		$sql="DELETE emails WHERE fecha_envio IS NOT null;";
		$this->execAdHoc($sql);
