					...
					_AJAX.Execute(_AJAX.formatFixedParameters(_json)).then(function (datajson) {
						if (datajson.status != undefined) {
							if (datajson.status == "OK") {
								...
								alert("Resultado No OK. Hubo un error procesando los datos.");
								reject(datajson);
							}
						} else {
							alert("Resultado undefined. Hubo un error procesando los datos.");
							resolve(datajson);
						}
					});
				} catch (rex) {
					alert("Resultado excepcion. Hubo un error procesando los datos.");
					reject(rex);
				}
			});
	},
...
//					alert("antes de execute");
					...
					var ajaxRq = $.ajax({
						...
						error: function (xhr, ajaxOptions, thrownError) {alert("Resultado Error. Hubo un error procesando los datos: "+thrownError);reject(thrownError);},
						success: function (datajson) {
							_AJAX.onSuccessExecute(datajson, _json)
								.then(function (datajson) { resolve(datajson); })
								.catch(function (err) { alert("Resultado No OK. Hubo un error procesando los datos.");reject(err); });
						}
					});
				} catch (rex) {
					alert("Resultado excepcion. Hubo un error procesando los datos.");
	 ...
	 */
	UiDeleteAllEmails: function (_json) {
		return new Promise(
			function (resolve, reject) {
				_json["function"] = "deleteAllEmails";
				_json["module"] = "mod_disidentes";
				_json["table"] = "Sac_emails";
				_json["model"] = "Sac_emails";
				_AJAX._waiter = true;
	...
	UiReciptSearchLote: function (_json) {
		...
		return new Promise(
			function (resolve, reject) {
				...
				_json["table"] = "Sac_facturacion_recibos";
				_json["model"] = "Sac_facturacion_recibos";
				_json["method"] = uri_prefix+"api.backend/neocommand";
